...
Handlebars.registerHelper("isRecording", () => {
  return Meteor.Meetings.findOne().currentlyBeingRecorded;
});
...
Handlebars.registerHelper("isCurrentUserModeratorOrPresenter", () => {
  return BBB.isUserPresenter(getInSession('userId')) || BBB.getCurrentUser().user.role == "MODERATOR";
});
...
/**
 * For providing i18n into templates
 * @param {string} translation - translation key
 * @param {...(string|number)} [placeholdersArguments] - array of arguments in which every odd
 *                                                     element is placeholder and next element
 *                                                     is his replacement value
 *  @return {string|null}
 */
Handlebars.registerHelper("i18n", (translation, ...placeholdersArguments) => {
  if(!i18n){
      return null;
  }
  //removes last argument which is provided by handlebars and not needed for this function
  var placeholdersArr = Array.prototype.slice.call(placeholdersArguments, 0, -1);
  if(!placeholdersArr.length){
    return i18n.get(translation);
  } else {
    var placeholders = {};
    for (var i = 0; i < placeholdersArr.length; i+=2) {
        placeholders[placeholdersArr[i]] = placeholdersArr[i+1]
    }
    return i18n.trans(translation, placeholders);
  }
...
this.onLoadComplete = function() {
  let ref;
  document.title = `YouSeeU Sync Meeting ${(ref = BBB.getMeetingName()) != null ? ref : 'HTML5'}`;
  setDefaultSettings();
