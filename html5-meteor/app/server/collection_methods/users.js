...
Meteor.methods({
 ...
 /**
  * Sends to server message with lock viewers settings
  *
  * @param {Object} settings - lock settings
  * @param {boolean} settings.disable_public_chat - disable public chat for viewers
  * @param {boolean} settings.locked_layout - locks viewers layout
  * @param {boolean} settings.disable_private_chat - disables private chat for viewer
  * @param {boolean} settings.disable_camera - disables camera sharing for viewers
  * @param {boolean} settings.disable_microphone - disables microphone fot viewers
  * @param {boolean} settings.lock_on_join -apply lock settings fot all new viewers
  * @param {'false'} settings.lock_on_join_configurable
  * @param {string} meetingId - id of current meeting
  * @param {string} userId - id of current user
  * @param {string} authToken - token for checking permissions
  */
  sendViewersLockSettings (settings, meetingId, userId, authToken){
    let message;
    if(isAllowedTo('lockUsers', meetingId, userId, authToken)) {
      message = {
        "payload": {
          "settings": settings,
          "userid": userId,
